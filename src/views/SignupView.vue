  ...
  <div class="flex min-h-screen items-center justify-center bg-background px-4">
    <div class="w-full max-w-sm">
      <div class="text-center">
        ...
        <h1 class="font-display mt-4 text-2xl font-extrabold tracking-tight">Create account</h1>
        <p class="mt-1 text-sm text-text-secondary">Set up your merchant console.</p>
      </div>

      <form @submit.prevent="onSubmit" class="mt-8 space-y-4">
        <label class="block">
          <span class="mb-1.5 block text-[11px] font-semibold uppercase tracking-[0.14em] text-text-secondary">Business name</span>
          <input
            v-model="name"
            type="text"
            placeholder="Olive & Ash"
      ...
      <div class="mt-6 flex items-center justify-between text-sm">
        <RouterLink to="/login" class="font-medium text-text-secondary transition hover:text-foreground">
